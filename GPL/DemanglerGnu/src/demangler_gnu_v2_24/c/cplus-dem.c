...
int cplus_demangle_opname(const char *opname, char *result, int options)
{
  ...
  if (opname[0] == '_' && opname[1] == '_' && opname[2] == 'o' && opname[3] == 'p')
  {
    /* ANSI.  */
  ...
  else if (opname[0] == '_' && opname[1] == '_' && ISLOWER((unsigned char)opname[2]) && ISLOWER((unsigned char)opname[3]))
  {
    if (opname[4] == '\0')
    {
      /* Operator.  */
      ...
      {
        if (strlen(optable[i].in) == 2 && memcmp(optable[i].in, opname + 2, 2) == 0)
        {
          strcat(result, "operator");
    ...
    else
    {
      if (opname[2] == 'a' && opname[5] == '\0')
      {
        ...
        for (i = 0; i < ARRAY_SIZE(optable); i++)
        {
          if (strlen(optable[i].in) == 3 && memcmp(optable[i].in, opname + 2, 3) == 0)
          {
            strcat(result, "operator");
            strcat(result, optable[i].out);
            ret = 1;
  ...
  else if (len >= 3 && opname[0] == 'o' && opname[1] == 'p' && strchr(cplus_markers, opname[2]) != NULL)
  {
        ...
        && memcmp(opname + 3, "assign_", 7) == 0)
    {
      ...
      {
        len1 = len - 10;
        if ((int)strlen(optable[i].in) == len1 && memcmp(optable[i].in, opname + 10, len1) == 0)
        {
          strcat(result, "operator");
          strcat(result, optable[i].out);
          strcat(result, "=");
    ...
    else
    {
      ...
      {
        len1 = len - 3;
        if ((int)strlen(optable[i].in) == len1 && memcmp(optable[i].in, opname + 3, len1) == 0)
        {
          strcat(result, "operator");
          strcat(result, optable[i].out);
          ret = 1;
  ...
  else if (len >= 5 && memcmp(opname, "type", 4) == 0 && strchr(cplus_markers, opname[4]) != NULL)
  {
    /* type conversion operator */
    tem = opname + 5;
    if (do_type(work, &tem, &type))
...
cplus_mangle_opname(const char *opname, int options)
{
  ...
  for (i = 0; i < ARRAY_SIZE(optable); i++)
  {
    if ((int)strlen(optable[i].out) == len && (options & DMGL_ANSI) == (optable[i].flags & DMGL_ANSI) && memcmp(optable[i].out, opname, len) == 0)
      return optable[i].in;
  }
  return (0);
}
...
ada_demangle(const char *mangled, int option ATTRIBUTE_UNUSED)
{
  ...
  while (1)
  {
    ...
    if (ISLOWER(*p))
    {
      ...
      do
        *d++ = *p++;
      while (ISLOWER(*p) || ISDIGIT(*p) || (p[0] == '_' && (ISLOWER(p[1]) || ISDIGIT(p[1]))));
    }
    else if (p[0] == 'O')
    {
      /* An operator name.  */
      static const char *const operators[][2] =
          {{"Oabs", "abs"}, {"Oand", "and"}, {"Omod", "mod"}, {"Onot", "not"}, {"Oor", "or"}, {"Orem", "rem"}, {"Oxor", "xor"}, {"Oeq", "="}, {"One", "/="}, {"Olt", "<"}, {"Ole", "<="}, {"Ogt", ">"}, {"Oge", ">="}, {"Oadd", "+"}, {"Osubtract", "-"}, {"Oconcat", "&"}, {"Omultiply", "*"}, {"Odivide", "/"}, {"Oexpon", "**"}, {NULL, NULL}};
      int k;
                        ...
                        const char **mangled, string *s)
{
 ...
 multi-digit numbers.  */
    ...
    if (value != -1)
    {
      ...
      if ((value > 9 || multidigit_without_leading_underscore) && !leave_following_underscore && **mangled == '_')
        (*mangled)++;
       ...
       int n, const char **anchor, const char **args)
{
  ...
  {
    if ((*anchor = strstr(mangled, "__tm__")) || (*anchor = strstr(mangled, "__ps__")) || (*anchor = strstr(mangled, "__pt__")))
    {
      int len;
      *args = *anchor + 6;
      len = consume_count(args);
                          ...
                          string *declp, const char *scan)
{
  ...
  if (ARM_DEMANGLING || LUCID_DEMANGLING || HP_DEMANGLING || EDG_DEMANGLING || strstr(scan + 2, "__") == NULL)
 ...
 to do.  */
  ...
  else if ((*mangled)[0] == '_' && (strchr("0123456789Qt", (*mangled)[1]) != NULL) && (p = strpbrk(*mangled, cplus_markers)) != NULL)
  {
    ...
    default:
      ...
      if (n > 10 && strncmp(*mangled, "_GLOBAL_", 8) == 0 && (*mangled)[9] == 'N' && (*mangled)[8] == (*mangled)[10] && strchr(cplus_markers, (*mangled)[8]))
      {
...
remember_type(struct work_stuff *work, const char *start, int len)
{
  ...
  if (work->ntypes >= work->typevec_size)
  {
    ...
    {
      work->typevec_size *= 2;
      work->typevec = XRESIZEVEC(char *, work->typevec, work->typevec_size);
    }
  }
...
remember_Ktype(struct work_stuff *work, const char *start, int len)
{
  ...
  if (work->numk >= work->ksize)
  {
    ...
    {
      work->ksize *= 2;
      work->ktypevec = XRESIZEVEC(char *, work->ktypevec, work->ksize);
    }
  }
  tem = XNEWVEC(char, len + 1);
  memcpy(tem, start, len);
...
register_Btype(struct work_stuff *work)
{
  ...
  if (work->numb >= work->bsize)
  {
    ...
    {
      work->bsize *= 2;
      work->btypevec = XRESIZEVEC(char *, work->btypevec, work->bsize);
    }
  }
  ret = work->numb++;
                       ...
                       string *declp, const char *scan)
{
 ...
 when we recover the class name from the signature.  */
  ...
  else if (declp->b[0] == '_' && declp->b[1] == '_' && ISLOWER((unsigned char)declp->b[2]) && ISLOWER((unsigned char)declp->b[3]))
  {
    if (declp->b[4] == '\0')
    {
      /* Operator.  */
...
int main(int argc, char **argv)
{
  ...
  while ((c = getopt_long(argc, argv, "_hinps:tv", long_options, (int *)0)) != EOF)
  {
    ...
    case '_':
      ...
      break;
    case 's':
      current_demangling_style = cplus_demangle_name_to_style(optarg);
