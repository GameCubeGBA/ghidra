...
public class SkeletonFileSystem implements GFileSystem {
	...
	public ByteProvider getByteProvider(GFile file, TaskMonitor monitor) throws IOException, CancelledException {
		...
		// TODO: Get an ByteProvider for a file. The following is an example of how the
		// metadata
		// might be used to get an sub-ByteProvider from a stored provider offset.
		MyMetadata metadata = fsih.getMetadata(file);
		return (metadata != null) ? new ByteProviderWrapper(provider, metadata.offset, metadata.size, file.getFSRL())
				: null;
	}
	...
	public static class MyFileSystemFactory
			implements GFileSystemFactoryByteProvider<SkeletonFileSystem>, GFileSystemProbeByteProvider {

		@Override
		public SkeletonFileSystem create(FSRLRoot targetFSRL, ByteProvider byteProvider, FileSystemService fsService,
