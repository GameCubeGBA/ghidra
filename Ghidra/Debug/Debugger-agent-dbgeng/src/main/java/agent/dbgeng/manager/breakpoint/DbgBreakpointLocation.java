...
public class DbgBreakpointLocation {
	 ...
	 * @param processIds a list of inferior IDs to which this location applies
	 */
	DbgBreakpointLocation(long number, long sub, boolean enabled, String addr,
			List<DebugProcessId> processIds) {
		this.number = number;
		this.sub = sub;
	...
	public long addrAsLong() {
		return Long.parseUnsignedLong(addr, 16);
	}
	...
	@Override
	public int hashCode() {
		return Objects.hash(number, sub, enabled, addr, processIds);
	}
