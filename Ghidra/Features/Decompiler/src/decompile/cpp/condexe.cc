...

{
  ...
  if (op->code() == CPUI_RETURN)
  {
    if ((op->numInput() < 2) || (op->getIn(1) != vn))
      return false;         // Only test for flow thru to return value
...

{
  ...
  for (zeroSlot = 0; zeroSlot < 2; ++zeroSlot)
  {
    ...
    if (copyop->code() != CPUI_COPY)
      continue; // Multiequal must have CPUI_COPY input
...

{
  ...
  if (!zerovn->isConstant())
    return false;
  if (zerovn->getOffset() != 0)
    return false; // Verify we are comparing to zero
...

{
  ...
  if (branch.op->getOut()->loneDescend() != op)
    return 0; // Must only be one use of MULTIEQUAL, because we rewrite it
  branch.discoverPathIsTrue();
  if (!branch.discoverConditionalZero(vn))
    return 0;
  if (branch.zeroPathIsTrue)
    return 0; // true condition (vn == 0) must not go to zero set
...

{
  ...
  if (branch0.condBlock == branch1.condBlock)
  {
    if (branch0.zeroBlock == branch1.zeroBlock)
      return 0; // zero sets must be along different paths
  }
  else
