	...
	public ConditionResult doRun(TaskMonitor monitor) {
		...
		if (!monitor.isCancelled()) {
			...
			if (percent > threshold) {
				...
				warnings.append(sourceProgram.getDomainFile().getName()).append(" and ").append(destinationProgram.getDomainFile().getName()).append(" have ").append(numSource).append(" and ").append(numDest).append(" no-return functions respectively,").append("\n");
