 ...
 * implements {@code ProgramTableModel} and the <code>setGoToService()</code> method
 * has been called with a valid reference.  When both of these conditions are met, then the 
 * table will navigate on a user's double-click or on an {@code Enter} key press.  Also, if
...
public class GhidraTable extends GTable {
	 ...
	 * the underlying model does not implement {@code ProgramTableModel}.
	 ...
	 * nothing if this table's {@code TableModel} is not an instance of {@link ProgramTableModel}.
	 ...
	 * navigate if {@link #navigateOnSelection} is {@code false}.
	 */
