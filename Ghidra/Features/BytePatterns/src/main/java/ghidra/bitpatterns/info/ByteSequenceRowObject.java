...
public class ByteSequenceRowObject {
	...
	private static List<ByteSequenceRowObject> getRowObjectsForLengthFilteredSeqs(
		...
		for (Map.Entry<String, Integer> entry : byteSeqCounts.entrySet()) {
            String bytes = entry.getKey();
	...
	private static List<ByteSequenceRowObject> getRowObjectsForPathFilteredSeqs(
		...
		for (Map.Entry<BytesAndDisassembly, Integer> entry : bytesAndDisCount.entrySet()) {
            BytesAndDisassembly bytesAndDisassembly = entry.getKey();
            Integer count = entry.getValue();
